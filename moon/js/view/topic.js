...
view.topic.next_prev = function (n, max_n) {
    // max_n = 11
    // n == [0, .., 11]
    // n_veiw = [1, ..., 12]
    ...
    if (n <= 0) {n_prev = max_n;}
    if (n + 1 >= max_n) {n_next = 1;}

    d3.select("#topic_view_next")
        .on("click", function (w) {
            d3.event.preventDefault();
            view.dfb().set_view("/topic/" + n_next + search_params.getUrl());
    });
    d3.select("#topic_view_prev")
        .on("click", function (w) {
            d3.event.preventDefault();
            view.dfb().set_view("/topic/" + n_prev + search_params.getUrl());
    });
};

view.topic.words = function (words, t, selectedWords) {
    var trs_w, td, checkbox, dictWords = {}, i;
    ...
    checkbox.attr('type', 'checkbox')
        .attr('name',  'word')
        .attr('value', function (w) { return w.word; })
        .attr('checked', function (w) {
            if (w.word in dictWords) {
                return true;
            } else {
                return undefined;
            }
        ...
        .on("change", function (w) {
            search_params.set_word(w.word, this.checked);
            //view.updating(true);
            view.dfb().set_view(
...
view.topic.authors = function (authors, t, selectedAuthors) {
    ...
    checkbox.attr('type', 'checkbox')
        ...
        .attr('checked', function (author) {
            if (dictAuthors.hasOwnProperty(author.author)) {
                return true;
            } else {
                return undefined;
            }
    ...
    trs_w.append("td").append("a")
        .text(function (w) {
            if (w.author.length < 10) {
                return w.author;
            } else {
                return w.author.substring(0, 10);
            }
        });
    //view.dirty("topic/words", false);
};
...
var SearchParams = function () {
    var that = { },
        ...
        hWords = { },
        set_word, // set (or delete) author
        set_words, // clear, and set authors (from url)
    ...
    set_author = function (author, set_remove) {
        author = author.trim();
        if (set_remove) {
            if (!(hAuthors.hasOwnProperty(author))) {
                hAuthors[author] = 1;
            } else {
                hAuthors[author] = 1;
            }
    ...
    set_authors = function(params) {
        hAuthors = { };
        var keys;
        if (params.hasOwnProperty('authors')) {
    ...
    set_word = function (word, set_remove) {
        word = word.trim();
        if (set_remove) {
            if (!(hWords.hasOwnProperty(word))) {
                hWords[word] = 1;
            } else {
                hWords[word] = 1;
            }
    ...
    that.set_word = set_word;

    set_words = function(params) {
        hWords = { };
        var keys;
        if (params.hasOwnProperty('words')) {
            keys = params['words'].split(',');
            keys.forEach(function (item) {hWords[item] = 1;});
        }
    };
    that.set_words = set_words;

    getUrl = function () {
        var a, params = [];

        a = Object.keys(hAuthors);
        if (a.length > 0) {
            params.push("authors=" + a.join(","));
        }
        a = Object.keys(hWords);
        if (a.length > 0) {
            params.push("words=" + a.join(","));
        }
        if (params.length > 0) {
            return "?" + params.join('&');
        } else {
