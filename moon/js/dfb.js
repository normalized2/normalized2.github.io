...
my.views.set("topic", function (t_user, year, searchParams) {
    ...
    // topic word subview
    var selectedWords = [];
    if (searchParams['words'] !== undefined) {
        selectedWords = searchParams['words'].split(',');
    }
    view.topic.words(words, t, selectedWords);
...
//    var meta_doc = my.m.meta(doc);
    ...
    if (year === undefined) {
        my.m.topic_docs(t, VIS.topic_view.docs,
            utils.parsedParams2query(searchParams),
            view_top_docs);
    } else {
...
my.views.set("doc", function (d) {
    ...
    my.m.doc_topics(doc, my.m.n(), function (ts) {
        ...
        view.doc({
            ...
            url: my.bib.url(meta_doc, VIS.url_pattern),
