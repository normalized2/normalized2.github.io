...
topic_docs = function (t, n, searchQuery) {
    return topic_docs_conditional(t, undefined, undefined, n, searchQuery);
};
...
topic_docs_conditional = function (t, v, key, n, searchQuery) {
    var p0 = my.dt.p[t],
        p1 = my.dt.p[t + 1],
        p,
        docs = [ ],
    ...
    searchParams = utils.parseParams(searchQuery);

    if (searchParams.hasOwnProperty('authors')) {
        selectedAuthors = searchParams.authors;
    } else {
        selectedAuthors = '';
    }
    ...
    if (searchParams.hasOwnProperty('words')) {
        selectedWords = searchParams.words;
        if (selectedWords !== '') {
            selectedWords = selectedWords.split(',');
        } else {
            selectedWords = [];
            }
    } else {
    ...
    filter = function (v, key, doc, dictAuthors) {
        var res, lemmas, meta_doc;
        res = (v === undefined);  // then: unconditional top docs
        if (!res) {
            res = (my.doc_categories[v][doc] === key);
        }
        if (res && (Object.keys(dictAuthors).length !== 0)) {
            res = (dictAuthors.hasOwnProperty(my.doc_categories.author[doc]));
        }
        if (res && (selectedWords.length !== 0)) {
            res = false;
            lemmas = my.doc_lemmas[doc];
            if ((lemmas !== undefined) && (lemmas !== '')) {
                lemmas = lemmas.split(' ');
                //res = utils.isIntersected(selectedWords, lemmas);
                res = utils.isSuperset(selectedWords, lemmas);
                }
            }
        return res;
    };
...
onmessage = function (e) {
    if (e.data.what === "set_dt") {
    ...
    } else if (e.data.what === "set_doc_lemmas") {
        my.doc_lemmas = e.data.doc_lemmas;
        postMessage({
            what: "set_doc_lemmas",
            result: my.doc_lemmas !== undefined
        });
        ...
        postMessage({
            what: "topic_docs/" + e.data.t + "/" + e.data.n + "/" + e.data.searchQuery,
            result: topic_docs(e.data.t, e.data.n, e.data.searchQuery)
        });
        ...
        postMessage({
            what: "topic_docs_conditional/" + e.data.t + "/" + e.data.v + "/"
                + e.data.key + "/" + e.data.n + "/" + e.data.searchQuery,
